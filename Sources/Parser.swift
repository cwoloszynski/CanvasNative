...
//		ReferenceLink.self,
	...
	public static func parse(_ string: NSString, range: NSRange? = nil) -> [BlockNode] {
		return parse(string as String, range: range)
	}

	public static func parse(_ string: String, range: NSRange? = nil) -> [BlockNode] {
		var nodes = [BlockNode]()
		...
		nodes = calculatePositions(blocks: nodes)
	...
	private static func parseInlineMarkers(string: String, container: InlineMarkerContainer) -> [InlineMarkerPair] {
		...
		for (i, block) in blocks.enumerated() {
			...
			if var item = currentBlock as? OrderedListItem {
				if let last = lastIndentation, last > item.indentation {
					_ = orderedIndentations.removeValue(forKey: last)
				}
			...
			// Starting position
			if isContinuous(lhs: previousBlock, currentBlock), let nextPosition = previousBlock?.position.successor {
			...
			// Check for ending
			if !isContinuous(lhs: currentBlock, nextBlock) {
				if position == .top {
					position = .single
				} else {
					position = .bottom
